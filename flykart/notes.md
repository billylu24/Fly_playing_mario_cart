...
## 2026-09-15 恢复数据训练对照完成

详见 recovery_training_results.md。9 模型：三个配对采样种子 × A旧数据MSE+KL、B旧数据一半MSE+CE、C恢复数据一半MSE+CE；各128更新，有效监督数量和初始化匹配，CNN/动作头冻结。验证交叉熵选择checkpoint后，在19个预设新起点完成13策略×19×12=2,964局评估及13局顺序回归。

C与B贪心正常均为9/19、13/19、6/19，恢复数据未提高贪心成功率。seed0/2的C选回第0步；B三个种子均选回第0步。仅seed1的C实际选中128步，采样正常29/57→43/57，但固定图像9/57→18/57；贪心救回两个f515起点、丢掉两个f445起点。合计C采样96/171、B82/171，收益全部来自seed1。预设稳定收益门槛未通过，且seed0/2贪心不高于时间控制9/19。CNN贪心正常16/19、固定0/19、时间9/19，采样正常47/57。

训练后只读目标一致性诊断：冻结CNN与旧示范动作一致86.7%，恢复示范74.0%。重建CNN特征与模仿恢复动作存在目标取舍，但不是不可学习的证明。此结果只否定本轮固定方案的稳定收益，不证明恢复数据无用或所有训练后checkpoint无效。

下一步先做动作头适应诊断，冻结CNN/图/解码器，只调整动作头，并配同数据的直接CNN可训练动作头对照。另从原验证起点构造恢复验证集，同时监控旧行为保留，再决定是否减少恢复样本MSE权重或调整步长。不用本轮19个测试起点调参；暂不增加E/I或扩大RL。

全部来源哈希、监督计数、冻结参数、步数0与原模型一致性及回归检查通过。新测试画面与旧训练、恢复训练及原验证/测试无精确重复；仍是同一赛道，不能视为跨赛道泛化。

## 2026-09-15 冻结特征后的动作头适应

完成8个动作头训练、12策略×10新起点×12评估组合=1,440局，另12局顺序回归。仅训练2179个动作头参数，CNN/图/接口/重建解码器/归一化固定。旧数据与恢复混合头使用相同初始化和采样计划。原验证6起点生成恢复验证，18局基线+62局分支得到6条合格去重轨迹，但只覆盖2个验证起点。选模约束旧验证CE不超过初始105%。

fly恢复混合头贪心6/10、8/10、8/10，旧数据头6/10、7/10、6/10，固定图像分别3/10、4/10、4/10，时间均4/10。预设闭环门槛通过：合计22/30对19/30，三组不劣且均保留视觉收益，采样正常62/90对58/90。不过原冻结头已有21/30，混合头只比未适应头多1/30；graph2主要避免旧数据适应退步。直接CNN冻结/旧/混合贪心10/10、8/10、9/10，仍无图拓扑优势证据。

离线旧/恢复验证CE全部改善，但graph0与CNN的恢复CE改善未到预设10%；graph1/2达到。此轮证明动作头适应有小规模正向信号，不是解决了全部架构问题。下一步固定checkpoint扩大新起点复验，再改变动作头采样种子；暂不更改动力学/EI。

训练前缓存一致性检查发现批量CNN特征误差3.72e-5被原头归一化放大为0.0046logit；在任何训练前改为逐帧缓存，不放宽断言，数据采集保持不变。修正前后哈希与原因保留在cache_fix_amendment.json。缓存/部署、冻结参数、来源哈希和12次回归均通过。恢复验证及新测试与训练帧无精确重叠，但样本仍相关。完整报告head_adaptation_results.md。
